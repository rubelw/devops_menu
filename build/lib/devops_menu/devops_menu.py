...
import getpass
import inspect
from devops_menu.commons.terminal import TerminalHandler
from devops_menu.view import Terminal
from devops_menu.controller import CommandExecutor
from devops_menu.setting.setting import Setting
from devops_menu.logger import SystemLogger
from devops_menu.exceptions import AwsCliMenuError
...
def get_hostname():
    if (DEBUG):
        print('devops_menu.py - get_hostname()- caller:'+str(inspect.stack()[1][3]))
...
def get_username():
    if (DEBUG):
        print('devops_menu.py - get_username()- caller:'+str(inspect.stack()[1][3]))
...
def main(stdin=sys.stdin, stdout=sys.stdout, stderr=sys.stderr, keep_input_clean=True, timing=True):
    ...
    """
    if (DEBUG):
        print('########### Main function from devops_menu.py ###############')
        print('devops_meny.py - main()- caller:'+str(inspect.stack()[1][3]))
    ...
    try:
        ...
        if (DEBUG):
            print('#################### devops_menu.py - load config, resolve encoding and set base settings ###########')

        setting = base_setting.resolve_encoding(handler).lookup_config().load_config()

        print('########### devops_menu.py - calling CommandExecutor ###########')
